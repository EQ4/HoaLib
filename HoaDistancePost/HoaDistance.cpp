...
m_order(order), m_speakersDistance(speakersDistance), m_sourceDistance(sourceDistance), m_theta(0)
{
	...
	m_tmpOutputArray = new double[2*m_order+1];
	...
	ConvolutionArray = new FftConvolution*[2*m_order+1];
	
	for (int i = 0; i < 2*m_order+1; i++) {
		ConvolutionArray[i] = new FftConvolution(m_fftSize, m_fftSize/2,2,44100,&m_halfComplexImpulseResponses[abs(i-m_order)]);
	}
...
void HoaDistance::ChangeSourceDistance(double sourceDistance)
{
	...
	for (int i = 0; i < 2*m_order+1; i++) {
		ConvolutionArray[i]->changeTransferFunction(&m_halfComplexImpulseResponses[abs(i-m_order)]);
	}
...
{	
	for (int i = 0 ; i < 2*m_order+1; i++) {
		m_tmpOutputArray[i] = ConvolutionArray[i]->process(aSample[i]);
	}
