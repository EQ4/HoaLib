...
{	
	...
	c = class_new("Hoa.NFCpost~", (method)HoaEncodeNF_new, (method)HoaEncodeNF_free, (long)sizeof(t_HoaEncodeNF), 0L, A_GIMME, 0);
...
//	return w + 2;
	...
	for (int i = 0; i<n; i++) 
	{
		...
		tmp_output = x->HoaDistanceInstance->process(tmp_in_vector);
...
void *HoaEncodeNF_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	long inputNumber = 2*orderNumber+1;
